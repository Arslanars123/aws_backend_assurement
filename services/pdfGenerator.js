...
class PDFGenerator {
  ...
  async generateStaticDocumentChecklistPDF(
    companyId,
    projectId,
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.BACKEND_URL}/get-static-document-checklist-with-status?companyId=${companyId}&projectId=${projectId}&professionSubjectMatterId=${professionSubjectMatterId}`
      );
      ...
      for (const [sectionName, items] of Object.entries(sections)) {
        if (items.length > 0) {
          this.addSectionHeader(
            `Section ${sectionName} - ${this.getSectionTitle(sectionName)}`
          );
  ...
  async generateStaticReportRegistrationEntriesPDF(
    companyId,
    projectId,
    professionSubjectMatterId
  ) {
    try {
      // Fetch data from API
      const response = await fetch(
        `${process.env.BACKEND_URL}/get-static-report-registration-entries?companyId=${companyId}&projectId=${projectId}&professionSubjectMatterId=${professionSubjectMatterId}`
      );
      const data = await response.json();
      ...
      console.error(
        "Error generating static report registration entries PDF:",
        error
      );
      throw error;
    }
  ...
  async generateCompleteStaticReportPDF(
    companyId,
    projectId,
    professionSubjectMatterId
  ) {
    try {
      ...
      await this.generateStaticDocumentChecklistContent(
        companyId,
        projectId,
        professionSubjectMatterId
      );
      ...
      await this.generateStaticReportRegistrationEntriesContent(
        companyId,
        projectId,
        professionSubjectMatterId
      );
  ...
  async generateStaticDocumentChecklistContent(
    companyId,
    projectId,
    professionSubjectMatterId
  ) {
    try {
        ...
        for (const [sectionName, items] of Object.entries(sections)) {
          if (items.length > 0) {
            this.addSubsectionHeader(
              `7.${sectionName} ${this.getSectionTitle(sectionName)}`
            );
      ...
      console.error(
        "Error generating static document checklist content:",
        error
      );
      this.addText(
        "Error loading static document checklist data",
        10,
        "italic"
      );
    }
  ...
  async generateStaticReportRegistrationEntriesContent(
    companyId,
    projectId,
    professionSubjectMatterId
  ) {
    try {
      ...
      if (data.success && data.entries) {
        ...
        for (const [sectionName, sectionEntries] of Object.entries(sections)) {
          if (sectionEntries.length > 0) {
            this.addSubsectionHeader(
              `8.${sectionName} ${this.getSectionTitle(sectionName)}`
            );
      ...
      console.error(
        "Error generating static report registration entries content:",
        error
      );
      this.addText(
        "Error loading static report registration entries data",
        10,
        "italic"
      );
    }
