...
async function fetchDynamicData(
  ...
  try {
    ...
    const companyResponse = await fetch(
      `${baseUrl}/get-company-detail/${companyId}`
    );
    ...
    const projectResponse = await fetch(
      `${baseUrl}/get-project-detail/${projectId}`
    );
    ...
    const specialTextResponse = await fetch(
      `${baseUrl}/get-project-special-text?projectId=${projectId}`
    );
    ...
    const signaturesResponse = await fetch(
      `${baseUrl}/get-static-report-signatures`,
      {
        method: "POST",
    ...
    return {
      ...
      signatures: signatures.filter(
        (sig) => sig.subjectMatterId === subjectMatterId
      ),
    };
  } catch (error) {
...
async function loadHTMLTemplates(page1Path, page2Path, dynamicData = {}) {
  try {
    ...
    const cleanPage1 = page1HTML
      .replace(/<script[\s\S]*?<\/script>/gi, "")
      .replace(/onload="[^"]*"/gi, "")
      .replace(/onclick="[^"]*"/gi, "");
    const cleanPage2 = page2HTML
      .replace(/<script[\s\S]*?<\/script>/gi, "")
      .replace(/onload="[^"]*"/gi, "")
      .replace(/onclick="[^"]*"/gi, "");
...
function injectServerData(html, data) {
  ...
  if (data.company) {
    processedHTML = processedHTML.replace(
      /id="company-name"[^>]*><\/[^>]*>/gi,
      `id="company-name">${data.company.name || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="company-name-full"[^>]*><\/[^>]*>/gi,
      `id="company-name-full">${data.company.name || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="company-address"[^>]*><\/[^>]*>/gi,
      `id="company-address">${data.company.address || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="company-post"[^>]*><\/[^>]*>/gi,
      `id="company-post">${data.company.postalCode || ""} ${
        data.company.city || ""
      }</div>`
    );
    processedHTML = processedHTML.replace(
      /id="company-cvr"[^>]*><\/[^>]*>/gi,
      `id="company-cvr">${data.company.cvr || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="company-email"[^>]*><\/[^>]*>/gi,
      `id="company-email">${data.company.email || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="company-contact"[^>]*><\/[^>]*>/gi,
      `id="company-contact">${data.company.contactPerson || ""}</div>`
    );
  ...
  if (data.project) {
    processedHTML = processedHTML.replace(
      /id="project-name"[^>]*><\/[^>]*>/gi,
      `id="project-name">${data.project.name || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="project-id"[^>]*><\/[^>]*>/gi,
      `id="project-id">${data.project._id || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="project-name-full"[^>]*><\/[^>]*>/gi,
      `id="project-name-full">${data.project.name || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="project-address"[^>]*><\/[^>]*>/gi,
      `id="project-address">${data.project.address || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="project-post"[^>]*><\/[^>]*>/gi,
      `id="project-post">${data.project.postalCode || ""} ${
        data.project.city || ""
      }</div>`
    );
    processedHTML = processedHTML.replace(
      /id="project-contact"[^>]*><\/[^>]*>/gi,
      `id="project-contact">${data.project.contactPerson || ""}</div>`
    );
    processedHTML = processedHTML.replace(
      /id="project-startup"[^>]*><\/[^>]*>/gi,
      `id="project-startup">${
  ...
  if (
    data.specialText &&
    data.specialText.success &&
    data.specialText.data &&
    data.specialText.data.specialText
  ) {
    processedHTML = processedHTML.replace(
      /id="document-id"[^>]*>B3\.<\/[^>]*>/gi,
      `id="document-id">B3. ${data.specialText.data.specialText}</div>`
    );
  }
  ...
  if (data.signatures && data.signatures.length > 0) {
    data.signatures.forEach((sig) => {
      if (sig.signatureType === 1) {
        processedHTML = processedHTML.replace(
          /id="sig1-name"[^>]*><\/[^>]*>/gi,
          `id="sig1-name">${sig.name || ""}</div>`
        );
        processedHTML = processedHTML.replace(
          /id="sig1-date"[^>]*><\/[^>]*>/gi,
          `id="sig1-date">${
            sig.signatureDate
              ? new Date(sig.signatureDate).toLocaleDateString()
              : ""
          }</div>`
        );
        if (sig.signature) {
...
router.post("/generate-combined-pdf", async (req, res) => {
  ...
  try {
    ...
    const page1Path = path.join(
      __dirname,
      "../../final_static/second_page.html"
    );
    const page2Path = path.join(
      __dirname,
      "../../final_static/signatures_page.html"
    );
    ...
    const combinedHTML = await loadHTMLTemplates(
      page1Path,
      page2Path,
      dynamicData
    );
    ...
    console.log(
      `📊 Stats: Size: ${pdfBuffer.length} bytes, Render time: ${renderTime}ms, Total time: ${totalTime}ms`
    );
